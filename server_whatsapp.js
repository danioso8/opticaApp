...
const sessions = new Map(); // organizationId -> { sock, qr, status, retryCount, badMacErrors, streamErrors, error515Count }
const AUTH_DIR = path.join(__dirname, 'auth_sessions');

// Rate limiting por organización
const connectionAttempts = new Map(); // organizationId -> { count, firstAttempt, last515Error }
...
const MAX_CONNECTION_ATTEMPTS = 3; // Máximo 3 intentos en 30 minutos
const RATE_LIMIT_WINDOW = 1800000; // 30 minutos
...
// Función para verificar si se puede intentar conexión (rate limiting)
function canAttemptConnection(organizationId) {
    const now = Date.now();
    const attempts = connectionAttempts.get(organizationId);
    
    if (!attempts) {
        connectionAttempts.set(organizationId, { count: 1, firstAttempt: now, last515Error: null });
        return true;
    }
    
    // Si hubo un error 515, esperar 5 minutos
    if (attempts.last515Error && (now - attempts.last515Error) < 300000) {
        logger.warn(`Error 515 reciente para ${organizationId}. Debe esperar ${Math.ceil((300000 - (now - attempts.last515Error)) / 60000)} minutos más.`);
        return false;
    }
    ...
    if (attempts.count >= MAX_CONNECTION_ATTEMPTS) {
        logger.warn(`Rate limit alcanzado para ${organizationId}. Debe esperar ${Math.ceil((RATE_LIMIT_WINDOW - (now - attempts.firstAttempt)) / 60000)} minutos más.`);
        return false;
    }
...
// Función para registrar error 515
function record515Error(organizationId) {
    const attempts = connectionAttempts.get(organizationId) || { count: 0, firstAttempt: Date.now(), last515Error: null };
    attempts.last515Error = Date.now();
    connectionAttempts.set(organizationId, attempts);
    
    const session = sessions.get(organizationId);
    if (session) {
        session.error515Count = (session.error515Count || 0) + 1;
        logger.error(`Error 515 #${session.error515Count} para ${organizationId}`);
    }
}
...
async function createWhatsAppConnection(organizationId) {
    try {
        ...
        if (!canAttemptConnection(organizationId)) {
            logger.warn(`⏸️  Conexión bloqueada por rate limiting para ${organizationId}`);
            const session = sessions.get(organizationId);
            if (session) {
        ...
        const sock = makeWASocket({
            ...
            getMessage: async (key) => {
                // Ignorar mensajes de broadcast/newsletter para evitar errores
        ...
        sock.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                // Registrar error 515 específicamente
                if (statusCode === 515) {
                    record515Error(organizationId);
                    logger.error(`⚠️  Error 515 detectado - WhatsApp bloqueando temporalmente ${organizationId}. Esperar 5 minutos.`);
                }
                ...
                if (errorMsg.includes('Stream Errored') || 
                    ...
                    // Para error 515 (rate limit), esperar mucho más tiempo
                    if (statusCode === 515) {
                        logger.warn(`⏰ Error 515 detectado - WhatsApp puede estar bloqueando temporalmente. Esperando 5 minutos antes de reintentar.`);
                        setTimeout(() => {
                            logger.info(`🔄 Reintentando después de error 515 para ${organizationId}...`);
                            createWhatsAppConnection(organizationId);
                        }, 300000); // 5 minutos
                    }
                ...
                if (shouldReconnect) {
                    ...
                    if (retryCount <= maxRetries) {
                        // Para error 515, usar delay de 5 minutos. Para otros, usar backoff exponencial
                        const baseDelay = statusCode === 515 ? 300000 : Math.min(1000 * Math.pow(2, Math.min(retryCount, 5)), 30000);
                        
                        logger.info(`🔄 Reconectando ${organizationId} en ${baseDelay}ms (intento ${retryCount}${hasStoredCreds ? ', sesión guardada' : ', sin credenciales'})`);
...
app.get('/api/status/:organization_id', authenticateAPI, (req, res) => {
    try {
        ...
        // No verificar readyState aquí porque puede ser inestable
        // Confiar en los eventos de conexión/desconexión de Baileys
